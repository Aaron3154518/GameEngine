...
RectData& RectData::set(Rect r, int thickness, bool center) {
	...
	if (center) {
		...
		int dw = 2 * thickness;
		if (dw > 0) { r2.resize(r2.w + dw, r2.h + dw, true); } 		else { r1.resize(r1.w + dw, r1.h + dw, true); }
	}
...
CircleData& CircleData::set(SDL_Point _c, int r, int thickness, bool center) {
	c = _c;
	r = std::abs(r);
	if (center) { r2 = r + std::abs(thickness) / 2; } else { r2 = thickness < 0 ? r : r + thickness; }
...
ProgressBar& ProgressBar::set(double amnt, double cap, bool log) {
	...
	if (cap < 0) { cap = 0.; }
	if (log) {
		if (amnt > 1) { amnt = log10(amnt); }
		else { amnt = 0; }
		if (cap > 1) { cap = log10(cap); }
		else { cap = 0; }
	}
