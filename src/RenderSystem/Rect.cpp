...
void Rect::setPos(const Rect& r, Rect::Align mode) {
	setPos(r, mode, mode);
}

void Rect::setPos(const Rect& r, Rect::Align xMode, Rect::Align yMode) {
	switch (xMode) {
		case Align::CENTER: setCX(r.cX()); break;
		case Align::BOT_RIGHT: setX2(r.x2()); break;
		default: x = r.x; break;
	}
	switch (yMode) {
		case Align::CENTER: setCY(r.cY()); break;
		case Align::BOT_RIGHT: setY2(r.y2()); break;
		default: y = r.y; break;
	}
}
